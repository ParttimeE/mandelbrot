...
// Wrap the color calculator with a background color function
export const getColorFnWithBackground = (colorCalculator: (i: number, m: number) => Color) => {
  return (backgroundColor: Color) => {
    return (i: number, m: number) => {
      if (i === m) return backgroundColor;
      return colorCalculator(i, m);
    }
  }
}
...
// Modulo-based color calculation with customizable divisors for each color channel
...
export const getBaseFactorColorCalculation = (redBaseValue: number, greenBaseValue: number, blueBaseValue: number) => {
  return (i: number, m: number) => {
    const colorRatio = 1 - (i / m);
